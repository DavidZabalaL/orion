...
export async function crearSiniestro(fd: FormData) {
  ...
  await prisma.siniestro.create({
    data: {
      ...
      huboArrastre: fd.get("huboArrastre") === "SI",
      costoArrastre: (fd.get("costoArrastre") as string) ? Number(fd.get("costoArrastre")) : null,
      costoReparacion: (fd.get("costoReparacion") as string) ? Number(fd.get("costoReparacion")) : null,
      estatus: "ABIERTO",
