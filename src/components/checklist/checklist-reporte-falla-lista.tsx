...
export function ChecklistReporteFallaLista({ checklists }: { checklists: ChecklistReporteFallaRow[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {filtrados.length === 0 ? (
        <EmptyState>Sin reportes de falla capturados hoy.</EmptyState>
      ) : (
        <Table headers={["Hora", "Unidad", "Tipo de falla", "Departamento", ""]} minWidth={720}>
          {filtrados.map((c) => {
            ...
            return (
              <Fragment key={c.id}>
                <tr style={{ borderBottom: expandido === c.id ? "none" : "1px solid var(--field-border)" }}>
                  ...
                  <td className="px-4 py-3" style={{ fontFamily: "var(--font-ui)", fontSize: "var(--text-base)", color: "var(--field-text)" }}>{respuestas.tipo_falla ?? "—"}</td>
                  <td className="px-4 py-3" style={{ fontFamily: "var(--font-ui)", fontSize: "var(--text-base)", color: "var(--field-text)" }}>{respuestas.departamento ?? "—"}</td>
